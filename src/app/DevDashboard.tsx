...
export function ConnectButton() {
  ...
  if (error) {
    return (<div style={{ fontSize: 12, color: '#ef4444' }}>{error}</div>)
  }
  if (address) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <button onClick={disconnect} style={{ fontSize: 12, padding: '6px 10px', borderRadius: 8, border: '1px solid #1e1e2e', background: 'transparent', color: '#64748b', cursor: 'pointer' }}>✕</button>
      </div>
    )
  }
  return (
...
export function DevDashboardTab() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined' && !(window as any).ethereum) setNoWallet(true)
  }, [])
  const loadDevData = useCallback(async (addr: string) => {
    setLoading(true)
    try {
      const balHex = await rpcCall('eth_getBalance', [addr, 'latest'])
      // Arc: native USDC uses 18 decimals (gas accounting), ERC-20 uses 6 decimals
      // Both interfaces represent the same underlying asset
      // eth_getBalance returns 18-decimal value, divide by 1e18 to get USDC amount
      ...
      const blockNums = Array.from({ length: 30 }, (_, i) => Math.max(0, latest - scanRange + i * step))
      const blocks = await Promise.all(blockNums.map(n => rpcCall('eth_getBlockByNumber', ['0x' + n.toString(16), true])))
      const txs: DevTx[] = []
      ...
      for (const block of blocks) {
        ...
        for (const tx of block.transactions) {
          ...
          txs.push({ hash: tx.hash, block: hexToNum(block.number), timestamp: hexToNum(block.timestamp), gasUsed, gasCost, type: isContract ? '📄 Contract Deploy' : '💸 Transfer', to: tx.to ?? 'Contract Creation' })
        }
      }
      setStats({ txCount: txs.length, totalGasUSDC: totalGas, contractsDeployed, balance, txs: txs.sort((a, b) => b.timestamp - a.timestamp).slice(0, 15) })
    } catch (e) { console.error(e) }
    setLoading(false)
  }, [])
  useEffect(() => { if (isConnected && address) loadDevData(address) }, [address, isConnected, loadDevData])
  if (noWallet) {
    return (
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.25rem' }}>
        ...
        <button onClick={() => address && loadDevData(address)} style={{ fontSize: 12, padding: '6px 14px', borderRadius: 8, border: '1px solid #1e1e2e', background: 'transparent', color: '#94a3b8', cursor: 'pointer' }}>↻ Refresh</button>
      </div>
      {loading ? (
        <div style={{ fontSize: 13, color: '#475569', textAlign: 'center', padding: '3rem' }}>Scanning Arc testnet for your activity...</div>
      ) : stats ? (
        <>
          ...
          <div style={{ background: '#13131a', border: '1px solid #1e1e2e', borderRadius: 12, padding: '1.25rem' }}>
            ...
            {stats.txs.length === 0 ? (
              <div style={{ fontSize: 13, color: '#475569', textAlign: 'center', padding: '2rem' }}>No transactions found in the last 500 blocks.</div>
            ) : (
                  ...
                  {stats.txs.map(tx => (
                    <tr key={tx.hash} style={{ borderTop: '1px solid #1e1e2e' }}>
                      <td style={{ padding: '8px 0', color: '#378ADD', fontFamily: 'monospace' }}>
                        <a href={`https://testnet.arcscan.app/tx/${tx.hash}`} target="_blank" rel="noopener noreferrer" style={{ color: '#378ADD', textDecoration: 'none' }}>
                          {tx.hash.slice(0, 8)}...{tx.hash.slice(-6)}
                        </a>
                      </td>
                      <td style={{ padding: '8px 0', color: '#94a3b8' }}>{tx.type}</td>
                      <td style={{ padding: '8px 0', color: '#64748b' }}>{timeAgo(tx.timestamp)}</td>
                      <td style={{ padding: '8px 0', textAlign: 'right', color: '#EF9F27', fontFamily: 'monospace' }}>{tx.gasCost.toFixed(8)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div style={{ fontSize: 13, color: '#ef4444', textAlign: 'center', padding: '2rem' }}>Failed to load data. Please try refreshing.</div>
      )}
    </div>
  )
}
